#!/usr/bin/env python3
"""
Simple Enhanced Signal Generator Test
Produces 2-3 high-quality signals with improved take profit consistency
"""
...
def main():
    """Main function to test simple enhanced signal generation."""
    logger = setup_logger()
    logger.info("Starting Simple Enhanced Signal Generator Test")
    
    try:
        # Initialize simple enhanced generator
        generator = SimpleEnhancedGenerator(balance=10000)
        ...
        if not signals:
            logger.warning("No signals generated. Market conditions may not be suitable.")
            print("\n❌ No signals generated. Market conditions may not be suitable.")
            return
        
        # Display results
        logger.info(f"Generated {len(signals)} enhanced signals:")
        print("\n" + "="*80)
        print("SIMPLE ENHANCED SIGNAL GENERATOR RESULTS")
        print("="*80)
        ...
        for i, signal in enumerate(signals, 1):
            ...
            print(f"  StochRSI %K: {indicators['stoch_rsi']['%K']:.2f}")
            
            print("\n" + "-" * 50)
        
        # Save signals to file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"simple_enhanced_signals_{timestamp}.json"
        ...
        print("\n" + "="*80)
        print("✅ Simple Enhanced Signal Generation Complete!")
        print("="*80)
        
    except Exception as e:
        logger.error(f"Error in simple enhanced signal generation: {e}")
        print(f"❌ Error: {e}")
        return 1
